...
import pandas
from .. import utils
from ..types import BioLectorModel, BLData, BLDParser, FilterTimeSeries, InvalidLotNumberError, LotInformationError, LotInformationMismatch, LotInformationNotFound, NoMeasurementData

class BioLector1Parser(BLDParser):

    def calibrate_with_lot(self, data, lot_number=None, temp=None):
        """Applies calibration.
        ...
        if lot_number is None and temp is None:
            if data.metadata['lot'] in {'UNKNOWN', 'UNKOWN'}:
                ...
                if cal_data is None:
                    warnings.warn('Lot information was found in the CSV file, but the calibration data was not found in the cache and the cache could not be updated. No calibration for pH and DO is applied.', LotInformationNotFound)
        ...
        if isinstance(lot_number, int) and isinstance(temp, int):
            if not data.metadata['lot'] in {'UNKNOWN', 'UNKOWN'}:
                ...
                if lot_number != lot_from_csv or temp != temp_from_csv:
                    warnings.warn(f'The lot information (lot_number={lot_number}, temp={temp}) provided mismatches with lot information found in the data file (lot_number={lot_from_csv}, temp={temp_from_csv}). ', LotInformationMismatch)
            cal_data = fetch_calibration_data(lot_number, temp)
            if cal_data is None:
                raise LotInformationError('Data for the lot information provided was not found in the cached file and we were unable to update it. If you want to proceed without calibration, pass no lot number and temperature')
            data = self.calibrate_with_parameters(data, **cal_data)
        return data

    def calibrate_with_parameters(self, data, cal_0=None, cal_100=None, phi_min=None, phi_max=None, pH_0=None, dpH=None):
    ...
    def parse(self, filepath, lot_number=None, temp=None, cal_0=None, cal_100=None, phi_min=None, phi_max=None, pH_0=None, dpH=None):
        ...
        data = BLData(model=BioLectorModel.BL1, environment=extract_environment(data, process_parameters, comments), filtersets=filtersets, references=references, measurements=measurements, comments=comments)
...
def extract_metadata(headerlines):
    ...
    metadata = {'filename': headerlines[0].split(';')[1].strip(), 'protocol': headerlines[1].split(';')[1].strip(), 'file_version': headerlines[2].split(';')[1].strip(), 'date_start': datetime.datetime.strptime(' '.join(headerlines[3].split(';')[1:3]).strip(), '%Y-%m-%d %H:%M:%S'), 'date_end': datetime.datetime.strptime(' '.join(L4[1:3]), '%Y-%m-%d %H:%M:%S'), 'last_reading': L4[5], 'timestamp': L4[7].strip(), 'device': headerlines[5][7:].strip(), 'user': L6[1], 'comment': L6[3].strip() if L6[3] != 'no comment available\n' else None, 'platetype': headerlines[7].split(';')[1], 'lot': headerlines[7].split(';')[3].strip(), 'mtp_rows': int(headerlines[8][9:-1]), 'mtp_columns': int(headerlines[9][12:-1]), 'filtersets': int(headerlines[10].split(';')[1]), 'reference_mode': headerlines[10].split(';')[3], 'multi_pmt': headerlines[10].split(';')[5]}
...
def extract_filtersets(headerlines):
    ...
    ocol_ncol_type = [('FILTERSET', 'filter_number', int), ('FILTERNAME', 'filter_name', str), ('FILTERNR', 'filter_id', str), (None, 'filter_type', str), ('EX [nm]', 'excitation', float), ('EM [nm]', 'emission', float), ('GAIN', 'gain', float), (None, 'gain_1', float), ('GAIN2', 'gain_2', float), ('PHASESTATISTICSSIGMA', 'phase_statistic_sigma', float), ('SIGNALQUALITYTOLERANCE', 'signal_quality_tolerance', float), (None, 'reference_gain', float), ('REFERENCE VALUE', 'reference_value', float), (None, 'calibration', str), ('EM2 [nm]', 'emission2', float)]
...
def extract_process_parameters(headerlines):
    ...
    for (l, line) in enumerate(headerlines):
        if line.startswith('FILTERSET;'):
            fs_start = l
            break
    process_parameters = {'temperature': float(headerlines[fs_start + 1].split(';')[13].strip()), 'humidity': float(headerlines[fs_start + 2].split(';')[13].strip()), 'O2': float(headerlines[fs_start + 3].split(';')[13].strip()), 'CO2': float(headerlines[fs_start + 4].split(';')[13].strip()), 'shaking': float(headerlines[fs_start + 5].split(';')[13].strip()), 'cycle_time': float(headerlines[fs_start + 6].split(';')[13].strip()), 'exp_time': float(headerlines[fs_start + 7].split(';')[13].strip())}
    return process_parameters

def extract_comments(dfraw, references):
    """This adds cycle numbers using timestamps from references."""
    ocol_ncol_type = [('TIME [h]', 'time', float), ('COMMENTS', 'user_comment', str)]
...
def extract_references(dfraw):
    ...
    ocol_ncol_type = [('cycle', 'cycle', int), ('TIME [h]', 'time', float), ('FILTERSET', 'filterset', int), ('AMPLITUDE', 'amp_1', float), ('amp_2', 'amp_2', float), ('PHASE', 'phase', float)]
...
def extract_measurements(dfraw):
    ...
    dfmes['cal'] = numpy.nan
    ocol_ncol_type = [('cycle', 'cycle', int), ('WELLNUM', 'well', str), ('FILTERSET', 'filterset', int), ('TIME [h]', 'time', float), ('AMPLITUDE', 'amp_1', float), ('amp_2', 'amp_2', float), ('amp_ref_1', 'amp_ref_1', float), ('amp_ref_2', 'amp_ref_2', float), ('PHASE', 'phase', float), ('cal', 'cal', float)]
    df = utils.__to_typed_cols__(dfmes, ocol_ncol_type)
    df = df.set_index(['filterset', 'cycle', 'well'])
    return df

def extract_environment(dfraw, process_parameters, comments):
    ocol_ncol_type = [('cycle', 'cycle', int), ('TIME [h]', 'time', float), (None, 'temp_setpoint', float), ('ACT TEMP [°C]', 'temp_up', float), (None, 'temp_down', float), (None, 'temp_water', float), ('ACT O2 [%]', 'o2', float), ('ACT CO2 [%]', 'co2', float), ('ACT HUMIDITY [rH]', 'humidity', float), (None, 'shaker_setpoint', float), (None, 'shaker_actual', float)]
...
def fetch_calibration_data(lot_number, temp):
    ...
    if calibration_info in parser:
        calibration_dict = {'cal_0': float(parser[calibration_info]['k0']), 'cal_100': float(parser[calibration_info]['k100']), 'phi_min': float(parser[calibration_info]['irmin']), 'phi_max': float(parser[calibration_info]['irmax']), 'pH_0': float(parser[calibration_info]['ph0']), 'dpH': float(parser[calibration_info]['dph'])}
