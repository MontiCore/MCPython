...
class GrowthRateResult:
    ...
    def __init__(self, *, t_data, t_segments, y, calibration_model, switchpoints, pmodel, theta_map):
    ...
    @property
    def detected_switchpoints(self):
    ...
    def sample(self, **kwargs):
        ...
        **sample_kwargs
        ...
        """
        sample_kwargs = dict(return_inferencedata=True, target_accept=0.95, init='adapt_diag', initvals=self.theta_map, tune=500, draws=500)
        sample_kwargs.update(kwargs)
        with self.pmodel:
            self._idata = pm.sample(**sample_kwargs)
        return

def _make_random_walk(name, *, init_dist, mu=0, sigma, nu=1, length, student_t, initval=None, dims=None):
    ...
    rw = pm.RandomWalk(name, init_dist=init_dist, innovation_dist=innov_dist, steps=length - 1, initval=initval, dims=dims)
    return rw

def _get_smoothed_mu(t, y, cm_cdw, *, clip=0.5):
...
def fit_mu_t(t, y, calibration_model, *, switchpoints=None, mcmc_samples=0, mu_prior=0, drift_scale, nu=5, x0_prior=0.25, student_t=None, switchpoint_prob=0.01, replicate_id='unnamed'):
    ...
    TD = len(t_data)
    TS = len(t_segments)
    coords = {'timepoint': numpy.arange(TD), 'segment': numpy.arange(TS)}
    with pm.Model(coords=coords) as pmodel:
        ...
        if len(t_switchpoints_known) > 0:
            ...
            for (i, t_switch) in enumerate(t_switchpoints_known):
                ...
                mu_segments.append(_make_random_walk(name, init_dist=init_dist, mu=0, sigma=drift_scale, nu=nu, length=i_len, student_t=student_t, initval=mu_guess[slc]))
                i_from += i_len
            ...
            mu_t = _make_random_walk('mu_t', init_dist=init_dist, mu=0, sigma=drift_scale, nu=nu, length=TS, student_t=student_t, initval=mu_guess, dims='segment')
        X0 = pm.LogNormal('X0', mu=numpy.log(x0_prior), sigma=1)
        Xt = pm.Deterministic('X', pt.concatenate([X0[None], X0 * pm.math.exp(pt.extra_ops.cumsum(mu_t * dt))]), dims='timepoint')
        calibration_model.loglikelihood(x=Xt, y=pm.ConstantData('backscatter', y, dims=('timepoint',)), replicate_id=replicate_id, dependent_key=calibration_model.dependent_key, dims='timepoint')
    with pmodel:
        theta_map = pm.find_MAP(maxeval=15000)
    if student_t:
        switchpoints_detected = detect_switchpoints(switchpoint_prob, t_data, pmodel, theta_map)
        switchpoints = {**switchpoints_detected, **switchpoints}
    result = GrowthRateResult(t_data=t_data, t_segments=t_segments, y=y, calibration_model=calibration_model, switchpoints=switchpoints, pmodel=pmodel, theta_map=theta_map)
...
def detect_switchpoints(switchpoint_prob, t_data, pmodel, theta_map):
    ...
    if len(cdf_evals_arr) != len(t_data) - 1:
        raise Exception(f'Failed to find all random walk segments. Found {len(cdf_evals_arr)}, expected {len(t_data) - 1}.')
    significance_mask = numpy.logical_or(cdf_evals_arr < switchpoint_prob / 2, cdf_evals_arr > 1 - switchpoint_prob / 2)
