...
def __to_typed_cols__(dfin, ocol_ncol_type):
...
def _reindex(dataframe, index_names):
...
def _concatenate_fragments(fragments, start_times):
...
def _last_well_in_cycle(measurements):
...
def _last_full_cycle(measurements):
...
def _parse_calibration_info(calibration_info):
    ...
    temp : int
        Process temperature
    """
    result = re.findall('(\\d*)-hc-Temp(\\d{2})', calibration_info)
