...
class UnivariateCubicSmoothingSpline(csaps.CubicSmoothingSpline):
    ...
    def derivative(self, order=1, *, epsilon=0.001):
...
def _normalize_smoothing_factor(method, smooth, x, y):
...
def _evaluate_smoothing_factor(smoothing_factor, timepoints, values, k, method):
    ...
    if len(values) < 3 * k:
        raise ValueError(f'Time series of {len(values)} elements is too short. Need at least a length of 3*k ({3 * k}).')
    ssrs = []
    for kshift in range(k):
        ...
        ssrs.append(_evaluate_spline_test_error(x=timepoints, y=values, train_idxs=train_mask, test_idxs=test_mask, smoothing_factor=smoothing_factor_arr, method=method))
    return float(numpy.mean(ssrs))

def _evaluate_spline_test_error(x, y, train_idxs, test_idxs, smoothing_factor, method):
...
def get_crossvalidated_spline(x, y, k_folds=5, method='us', bounds=(0.001, 1)):
    ...
    """
    opt = scipy.optimize.differential_evolution(_evaluate_smoothing_factor, bounds=[bounds], args=(x, y, k_folds, method))
...
def get_multiple_splines(fts, wells, k_folds=5, method='us', last_cycle=None):
...
def get_mue(bsdata, wells='all', blank='first', k_folds=5, method='us', last_cycle=None):
    ...
    filtertimeseries = FilterTimeSeries(pandas.DataFrame(time), pandas.DataFrame(mues))
