...
class BLData(dict):
    ...
    def get_unified_narrow_data(self, source_well='first', source_filterset='first', *, last_cycles=None):
        ...
        u_narrow = pandas.DataFrame(columns=['well', 'cycle', 'time'] + list(self.keys()))
        ...
        if last_cycles:
            for (well, last_cycle) in last_cycles.items():
                u_narrow.drop(u_narrow[(u_narrow.well == well) & (u_narrow.cycle > last_cycle)].index, inplace=True)
        return u_narrow

    def get_timeseries(self, filterset, well, *, last_cycle=None):
...
class FilterTimeSeries:
    ...
    def get_timeseries(self, well, *, last_cycle=None):
    ...
    def get_unified_dataframe(self, well=None):
