...
import pandas
from .. import utils
from ..types import BioLectorModel, BLData, BLDParser, FilterTimeSeries, FluidicsSource, IncompatibleFileError, InvalidLotNumberError
logger = logging.getLogger('blpro')
_MF_WELL_NUMC_TO_ID = {colnumber + rownumber * 8: f'{row}{column:02d}' for (colnumber, column) in enumerate(range(1, 9)) for (rownumber, row) in enumerate('CDEF')}
'Maps 0-based well numbers in MF mode by **C-style** counting order (→ then ↓) to alphanumeric well IDs.'
_MF_WELL_NUMF_TO_ID = {rownumber + colnumber * 4: f'{row}{column:02d}' for (rownumber, row) in enumerate('CDEF') for (colnumber, column) in enumerate(range(1, 9))}
'Maps 0-based well numbers in MF mode by **Fortran-style** counting order (↓ then →) to alphanumeric well IDs.'
_MF_WELL_NUMM_TO_ID = {1 + (colnumber + rownumber * 8): f'{row}{column:02d}' for (rownumber, row) in enumerate('CDEF') for (colnumber, column) in enumerate(reversed(range(1, 9)) if rownumber % 2 else range(1, 9))}
...
class BioLectorProParser(BLDParser):
    ...
    def parse(self, filepath, lot_number=None, temp=None, cal_0=None, cal_100=None, phi_min=None, phi_max=None, pH_0=None, dpH=None):
        (metadata, data) = parse_metadata_data(filepath)
        bld = BLData(model=BioLectorModel.BLPro, environment=extract_environment(data), filtersets=extract_filtersets(metadata), references=extract_references(data), measurements=extract_measurements(data), comments=extract_comments(data))
...
def _filter_datalines(datalines):
    """Filters out unnecessary intermediate header sections."""
    datalines = [dl for (l, dl) in enumerate(datalines) if len(dl) > 1 and dl[1] == ';']
    return datalines

def _parse_datalines(datalines):
    dfraw = pandas.read_csv(io.StringIO(''.join(datalines)), sep=';', low_memory=False, converters={'Filterset': str})
    return dfraw
...
def parse_metadata_data(fp):
    ...
    try:
        ...
        for (l, line) in enumerate(datalines):
            if line.count(';') <= n_allowed and (not ('\x10' in line or '\x00' in line)) and (not re.search('\\d*\\.\\d*\\.\\d*', line)):
...
def extract_filtersets(metadata):
    ...
    fname_lookup = {'201': 'Biomass', '202': 'pH(HP8)', '203': 'DO(PSt3)'}
    ...
    ocol_ncol_type = [('no', 'filter_number', int), ('name', 'filter_name', str), ('filter_id', 'filter_id', str), ('filter_type', 'filter_type', str), (None, 'excitation', float), (None, 'emission', float), ('gain', 'gain', float), ('gain_1', 'gain_1', float), ('gain_2', 'gain_2', float), (None, 'phase_statistic_sigma', float), (None, 'signal_quality_tolerance', float), ('reference_gain', 'reference_gain', float), ('reference_value', 'reference_value', float), ('calibration', 'calibration', str), (None, 'emission2', float)]
    return utils.__to_typed_cols__(df, ocol_ncol_type)

def extract_comments(dfraw):
    ocol_ncol_type = [('Cycle', 'cycle', int), ('Time', 'time', float), ('User_Comment', 'user_comment', str), ('Sys_Comment', 'sys_comment', str)]
    df = utils.__to_typed_cols__(dfraw[dfraw['Type'] == 'C'], ocol_ncol_type)
    return standardize(df)

def extract_parameters(dfraw):
    ocol_ncol_type = [('Cycle', 'cycle', int), ('Time', 'time', float)]
    df = utils.__to_typed_cols__(dfraw[dfraw['Type'] == 'P'], ocol_ncol_type)
    return standardize(df)

def extract_references(dfraw):
    ocol_ncol_type = [('Cycle', 'cycle', int), ('Filterset', 'filterset', int), ('Time', 'time', float), ('Amp_1', 'amp_1', float), ('Amp_2', 'amp_2', float), ('Phase', 'phase', float)]
...
def extract_measurements(dfraw):
    ocol_ncol_type = [('Cycle', 'cycle', int), ('Well', 'well', int), ('Filterset', 'filterset', str), ('Time', 'time', float), ('Amp_1', 'amp_1', float), ('Amp_2', 'amp_2', float), ('AmpRef_1', 'amp_ref_1', float), ('AmpRef_2', 'amp_ref_2', float), ('Phase', 'phase', float), ('Cal', 'cal', float)]
    df_M = dfraw[dfraw['Type'] == 'M']
    mask = (df_M['AmpRef_1'] == 'REFOVERLD') | (df_M['AmpRef_2'] == 'REFOVERLD')
    ndrop = sum(mask)
    if ndrop:
        cdrop = df_M[mask]['Cycle'].to_list()
        warnings.warn(f'Dropped {ndrop} measurement rows from cycles {cdrop} because they have REFOVERLD.', UserWarning)
    df_M = df_M[~mask]
    drop_idxs = []
    ...
    if ndrop:
        for (dropC, dropF) in drop_idxs:
            ...
            df_M = df_M[~mask]
            warnings.warn(f'Dropped cycle {dropC} filterset {dropF} because of non-monotonically increasing time values.', UserWarning)
    df = utils.__to_typed_cols__(df_M, ocol_ncol_type)
    df = df.set_index(['filterset', 'cycle', 'well'])
    return standardize(df)

def extract_environment(dfraw):
    ocol_ncol_type = [('Cycle', 'cycle', int), ('Time', 'time', float), (None, 'temp_setpoint', float), ('Temp_up', 'temp_up', float), ('Temp_down', 'temp_down', float), ('Temp_water', 'temp_water', float), ('O2', 'o2', float), ('CO2', 'co2', float), ('Humidity', 'humidity', float), (None, 'shaker_setpoint', float), ('Shaker', 'shaker_actual', float)]
    df = utils.__to_typed_cols__(dfraw[dfraw['Type'] == 'R'], ocol_ncol_type)
    return standardize(df)

def extract_fluidics(dfraw):
    ocol_ncol_type = [('Cycle', 'cycle', int), ('Well', 'well', int), ('Time', 'time', float), ('Reservoir', 'reservoir', FluidicsSource), ('MF_Volume', 'mf_volume', float), ('Temp_Ch4', 'volume', float)]
...
def extract_valves_module(dfraw):
    ocol_ncol_type = [('Cycle', 'cycle', int), ('Well', 'valve', str), ('Filterset', 'well', str), ('Time', 'volume_1', str), ('Amp_1', 'volume_2', str)]
...
def extract_diagnostics(dfraw):
    ...
    ocol_ncol_type = [('Cycle', 'cycle', int), ('Time', 'time', float), ('Temp_Ch4', 'temp_ch4', float), ('T_144', 't_144', float), ('T_180', 't_180', float), ('T_181_1', 't_181_1', float), ('T_192', 't_192', float), ('P_Ch1', 'p_ch1', float), ('P_Ch2', 'p_ch2', float), ('P_Ch3', 'p_ch3', float), ('T_Hum', 't_hum', float), ('T_CO2', 't_co2', float), ('X-Pos', 'x-pos', float), ('Y-Pos', 'y-pos', float), ('T_LED', 't_led', float), ('Ref_Int', 'ref_int', float), ('Ref_Phase', 'ref_phase', float), ('Ref_Gain', 'ref_gain', float), ('Ch1-MP', 'ch1_mp', float), ('Ch2-MF', 'ch2_mf', float), ('Ch3-FA', 'ch3_fa', float), ('Ch4-OP', 'ch4_op', float), ('Ch5-FB', 'ch5_fb', float)]
    df = utils.__to_typed_cols__(dff, ocol_ncol_type)
    return standardize(df)

def transform_into_filtertimeseries(metadata, measurements, filtersets, return_uncalibrated_optode_data):
    ...
    for fs in filtersets.itertuples():
        ...
        if filter_number not in measurements.index.get_level_values('filterset'):
            logger.warning('Skipped channel %s with name "%s" because no valid measurements are available.', fs.filter_type, fs.filter_name)
            continue
        dfm = measurements.xs(filter_number, level='filterset')
        mask = dfm.index.duplicated(keep='first')
        if any(mask):
            logger.warning('Duplicate filter %s measurements for (cycles, wells) %s.', filter_number, dfm[mask].index.to_list())
            dfm = dfm[~mask]
...
def fetch_calibration_data(lot_number, temp):
    ...
    if not element:
        if not utils.download_calibration_data():
            ...
            element = search_for_lot(calibration_file, lot_number)
    if not element:
        raise InvalidLotNumberError(f'Latest calibration information was downloaded from m2p-labs, but the provided lot number/temperature combination (lot_number={lot_number}, temp={temp}) could not be found. Please check the parameters.')
    cp = dict()
    ...
    calibration_dict = {'cal_0': cp['fCal0_m'] * temp + cp['fCal0_a'], 'cal_100': cp['fCal100_m'] * temp + cp['fCal100_a'], 'phi_min': cp['fMin_m'] * temp + cp['fMin_a'], 'phi_max': cp['fMax_m'] * temp + cp['fMax_a'], 'pH_0': cp['pH0_m'] * temp + cp['pH0_a'], 'dpH': cp['dpH_m'] * temp + cp['dpH_a']}
