...
from .types import BioLectorModel, BLData, BLDParser, FilterTimeSeries, IncompatibleFileError, InvalidLotNumberError, LotInformationError, LotInformationMismatch, LotInformationNotFound, NoMeasurementData
parsers = {(BioLectorModel.BL1, '3.3'): parsing.bl1.BioLector1Parser, (BioLectorModel.BLPro, '0.0.0'): parsing.blpro.BioLectorProParser}
...
def _parse(filepath, drop_incomplete_cycles, lot_number, temp, cal_0=None, cal_100=None, phi_min=None, phi_max=None, pH_0=None, dpH=None):
    ...
    data = parser.parse(filepath, lot_number=lot_number, temp=temp, cal_0=cal_0, cal_100=cal_100, phi_min=phi_min, phi_max=phi_max, pH_0=pH_0, dpH=dpH)
...
def parse(filepaths, *, drop_incomplete_cycles=True, lot_number=None, temp=None, cal_0=None, cal_100=None, phi_min=None, phi_max=None, pH_0=None, dpH=None):
    ...
    if isinstance(filepaths, Iterable) and (not isinstance(filepaths, str)):
        ...
        if len(fragments) > 1:
            for fs in data.keys():
                conc_times = utils._concatenate_fragments([f[fs].time + (fragment_start - start_times[0]).total_seconds() / 3600 for (f, fragment_start) in zip(fragments, start_times)], start_times)
