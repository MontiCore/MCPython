...
__all__ = ('evo_make_selection_array', 'evo_get_selection', 'evo_aspirate', 'evo_dispense', 'evo_wash')
MAX_DILUTOR_VOLUME = 950
'"Maximum dilutor volume in µL'

def evo_make_selection_array(rows, columns, wells):
...
def evo_get_selection(rows, cols, selected):
    ...
    for column in selected.transpose():
        if sum(column) >= 1:
            check += 1
    if check >= 2:
        raise ValueError('Wells from more than one column are selected.\nSelect only wells from one column per pipetting action.')
    return selection

def prepare_evo_aspirate_dispense_parameters(wells, *, labware_position, volume, liquid_class, tips, arm, max_volume=None):
    ...
    if isinstance(volume, list):
        ...
        if not len(volume) == len(tips) == len(wells_list):
            raise Exception(f'Invalid volume: Tips, wells, and volume lists have different lengths ({len(tips)}, {len(wells_list)} and {len(volume)}, respectively).')
        ...
        raise ValueError(f'Invalid volume: {volume}')
    volume_list = np.round(volume, decimals=2).tolist()
...
def evo_aspirate(*, n_rows, n_columns, wells, labware_position, volume, liquid_class, tips, arm=0, max_volume=np.nan):
    ...
    (wells, labware_position, volume, liquid_class, tips) = prepare_evo_aspirate_dispense_parameters(wells=wells, labware_position=labware_position, volume=volume, liquid_class=liquid_class, tips=tips, arm=arm, max_volume=max_volume)
    tip_selection = 0
    for tip in tips:
...
def evo_dispense(*, n_rows, n_columns, wells, labware_position, volume, liquid_class, tips, arm=0, max_volume=np.nan):
...
def prepare_evo_wash_parameters(*, tips, waste_location, cleaner_location, arm=0, waste_vol=3.0, waste_delay=500, cleaner_vol=4.0, cleaner_delay=500, airgap=10, airgap_speed=70, retract_speed=30, fastwash=1, low_volume=0):
    """Validates and prepares aspirate/dispense parameters.
...
def evo_wash(*, tips, waste_location, cleaner_location, arm=0, waste_vol=3.0, waste_delay=500, cleaner_vol=4.0, cleaner_delay=500, airgap=10, airgap_speed=70, retract_speed=30, fastwash=1, low_volume=0):
    ...
    low_volume : int
        Use pinch valves = 1, don't use them = 0
    """
    (tips, waste_location, cleaner_location, arm, waste_vol, waste_delay, cleaner_vol, cleaner_delay, airgap, airgap_speed, retract_speed, fastwash, low_volume) = prepare_evo_wash_parameters(tips=tips, waste_location=waste_location, cleaner_location=cleaner_location, arm=arm, waste_vol=waste_vol, waste_delay=waste_delay, cleaner_vol=cleaner_vol, cleaner_delay=cleaner_delay, airgap=airgap, airgap_speed=airgap_speed, retract_speed=retract_speed, fastwash=fastwash, low_volume=low_volume)
    tip_selection = 0
    for tip in tips:
        tip_selection += tip.value
