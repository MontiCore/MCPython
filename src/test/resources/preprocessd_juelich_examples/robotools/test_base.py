...
from robotools.worklists.utils import partition_by_column, partition_volume, prepare_aspirate_dispense_parameters
...
class TestWorklist:
    ...
    def test_parameter_validation(self):
        ...
        with pytest.raises(ValueError):
            prepare_aspirate_dispense_parameters(rack_label='thisisaveryverylongracklabelthatexceedsthemaximumlength', position=1, volume=15)
        with pytest.raises(ValueError):
            prepare_aspirate_dispense_parameters(rack_label='rack label; with semicolon', position=1, volume=15)
        ...
        with pytest.raises(ValueError):
            prepare_aspirate_dispense_parameters(rack_label='WaterTrough', position=1, volume=15, liquid_class=None)
        with pytest.raises(ValueError):
            prepare_aspirate_dispense_parameters(rack_label='WaterTrough', position=1, volume=15, liquid_class='liquid;class')
        prepare_aspirate_dispense_parameters(rack_label='WaterTrough', position=1, volume=15, liquid_class='valid liquid class')
        (_, _, _, _, tip, _, _, _, _) = prepare_aspirate_dispense_parameters(rack_label='WaterTrough', position=1, volume=15, tip=4)
        assert tip == 8
        (_, _, _, _, tip, _, _, _, _) = prepare_aspirate_dispense_parameters(rack_label='WaterTrough', position=1, volume=15, tip=Tip.T5)
        assert tip == 16
        (_, _, _, _, tip, _, _, _, _) = prepare_aspirate_dispense_parameters(rack_label='WaterTrough', position=1, volume=15, tip=(Tip.T4, 4))
        assert tip == 8
        (_, _, _, _, tip, _, _, _, _) = prepare_aspirate_dispense_parameters(rack_label='WaterTrough', position=1, volume=15, tip=[Tip.T1, 4])
        assert tip == 9
        (_, _, _, _, tip, _, _, _, _) = prepare_aspirate_dispense_parameters(rack_label='WaterTrough', position=1, volume=15, tip=[1, 4])
        assert tip == 9
        (_, _, _, _, tip, _, _, _, _) = prepare_aspirate_dispense_parameters(rack_label='WaterTrough', position=1, volume=15, tip=[1, Tip.T4])
        assert tip == 9
        (_, _, _, _, tip, _, _, _, _) = prepare_aspirate_dispense_parameters(rack_label='WaterTrough', position=1, volume=15, tip=Tip.Any)
        assert tip == ''
        with pytest.raises(ValueError, match='no Tip.Any elements are allowed'):
            prepare_aspirate_dispense_parameters(rack_label='WaterTrough', position=1, volume=15, tip=(Tip.T1, Tip.Any))
        ...
        with pytest.raises(ValueError, match='it may only contain int or Tip values'):
            prepare_aspirate_dispense_parameters(rack_label='WaterTrough', position=1, volume=15, tip=[1, 2.6])
        ...
        with pytest.raises(ValueError):
            prepare_aspirate_dispense_parameters(rack_label='WaterTrough', position=1, volume=15, rack_id=None)
        with pytest.raises(ValueError):
            prepare_aspirate_dispense_parameters(rack_label='WaterTrough', position=1, volume=15, rack_id='invalid;rack')
        with pytest.raises(ValueError):
            prepare_aspirate_dispense_parameters(rack_label='WaterTrough', position=1, volume=15, rack_id='thisisaveryverylongrackthatexceedsthemaximumlength')
        prepare_aspirate_dispense_parameters(rack_label='WaterTrough', position=1, volume=15, rack_id='1235464')
        with pytest.raises(ValueError):
            prepare_aspirate_dispense_parameters(rack_label='WaterTrough', position=1, volume=15, rack_type=None)
        with pytest.raises(ValueError):
            prepare_aspirate_dispense_parameters(rack_label='WaterTrough', position=1, volume=15, rack_type='invalid;rack type')
        with pytest.raises(ValueError):
            prepare_aspirate_dispense_parameters(rack_label='WaterTrough', position=1, volume=15, rack_type='thisisaveryverylongracktypethatexceedsthemaximumlength')
        prepare_aspirate_dispense_parameters(rack_label='WaterTrough', position=1, volume=15, rack_type='valid rack type')
        with pytest.raises(ValueError):
            prepare_aspirate_dispense_parameters(rack_label='WaterTrough', position=1, volume=15, forced_rack_type=None)
        with pytest.raises(ValueError):
            prepare_aspirate_dispense_parameters(rack_label='WaterTrough', position=1, volume=15, forced_rack_type='invalid;forced rack type')
        prepare_aspirate_dispense_parameters(rack_label='WaterTrough', position=1, volume=15, forced_rack_type='valid forced rack type')
        return
    ...
    def test_generic_transfer_raises_notimplemented(self):
...
class TestStandardLabwareWorklist:
    ...
    def test_aspirate(self, wl_cls):
        ...
        with wl_cls() as wl:
            ...
            assert wl == ['A;SourceLW;;;1;;50.00;;;;', 'A;SourceLW;;;4;;50.00;;;;', 'A;SourceLW;;;6;;50.00;;;;', 'C;second aspirate', 'A;SourceLW;;;7;;10.00;;;;', 'A;SourceLW;;;8;;20.00;;;;', 'A;SourceLW;;;9;;30.50;;;;']
            np.testing.assert_array_equal(source.volumes, [[150, 150, 190], [200, 200, 180], [200, 150, 169.5]])
            assert len(source.history) == 3
        return

    def test_aspirate_2d_volumes(self, wl_cls):
        source = Labware('SourceLW', rows=2, columns=3, min_volume=10, max_volume=200, initial_volumes=200)
        with wl_cls() as wl:
            wl.aspirate(source, source.wells[:, :2], volumes=np.array([[20, 30], [15.3, 17.53]]))
            assert wl == ['A;SourceLW;;;1;;20.00;;;;', 'A;SourceLW;;;2;;15.30;;;;', 'A;SourceLW;;;3;;30.00;;;;', 'A;SourceLW;;;4;;17.53;;;;']
            np.testing.assert_array_equal(source.volumes, [[180, 170, 200], [200 - 15.3, 200 - 17.53, 200]])
            assert len(source.history) == 2
        return

    def test_dispense(self, wl_cls):
        ...
        with wl_cls() as wl:
            ...
            assert wl == ['D;DestinationLW;;;1;;150.00;;;;', 'D;DestinationLW;;;3;;150.00;;;;', 'D;DestinationLW;;;5;;150.00;;;;', 'C;second dispense', 'D;DestinationLW;;;2;;10.00;;;;', 'D;DestinationLW;;;4;;20.00;;;;', 'D;DestinationLW;;;6;;30.50;;;;']
            np.testing.assert_array_equal(destination.volumes, [[150, 150, 150], [10, 20, 30.5]])
            assert len(destination.history) == 3
        return

    def test_dispense_2d_volumes(self, wl_cls):
        destination = Labware('DestinationLW', rows=2, columns=3, min_volume=10, max_volume=200)
        with wl_cls() as wl:
            wl.dispense(destination, destination.wells[:, :2], volumes=np.array([[20, 30], [15.3, 17.53]]))
            assert wl == ['D;DestinationLW;;;1;;20.00;;;;', 'D;DestinationLW;;;2;;15.30;;;;', 'D;DestinationLW;;;3;;30.00;;;;', 'D;DestinationLW;;;4;;17.53;;;;']
            np.testing.assert_array_equal(destination.volumes, [[20, 30, 0], [15.3, 17.53, 0]])
            assert len(destination.history) == 2
        return

    def test_skip_zero_volumes(self, wl_cls):
        source = Labware('SourceLW', rows=3, columns=3, min_volume=10, max_volume=200, initial_volumes=200)
        destination = Labware('DestinationLW', rows=2, columns=3, min_volume=10, max_volume=200)
        with wl_cls() as wl:
            wl.aspirate(source, ['A03', 'B03', 'C03'], [10, 0, 30.5])
            wl.dispense(destination, ['B01', 'B02', 'B03'], [10, 0, 30.5])
            assert wl == ['A;SourceLW;;;7;;10.00;;;;', 'A;SourceLW;;;9;;30.50;;;;', 'D;DestinationLW;;;2;;10.00;;;;', 'D;DestinationLW;;;6;;30.50;;;;']
            np.testing.assert_array_equal(source.volumes, [[200, 200, 190], [200, 200, 200], [200, 200, 169.5]])
            np.testing.assert_array_equal(destination.volumes, [[0, 0, 0], [10, 0, 30.5]])
            assert len(destination.history) == 2
        return

    def test_tip_selection(self, wl_cls):
        ...
        with wl_cls() as wl:
            ...
            assert wl == ['A;A;;;1;;10.00;;;1;', 'A;A;;;1;;10.00;;;2;', 'A;A;;;1;;10.00;;;4;', 'A;A;;;1;;10.00;;;8;', 'A;A;;;1;;10.00;;;16;', 'A;A;;;1;;10.00;;;32;', 'A;A;;;1;;10.00;;;64;', 'A;A;;;1;;10.00;;;128;', 'D;A;;;2;;10.00;;;1;', 'D;A;;;5;;10.00;;;2;', 'D;A;;;8;;10.00;;;4;', 'D;A;;;11;;10.00;;;8;', 'D;A;;;11;;10.00;;;16;', 'D;A;;;11;;10.00;;;32;', 'D;A;;;11;;10.00;;;64;', 'D;A;;;11;;10.00;;;128;']
        return

    def test_tip_mask(self, wl_cls):
        A = Labware('A', 3, 4, min_volume=10, max_volume=250)
        tips = [Tip.T1, Tip.T4, Tip.T7]
        with wl_cls() as wl:
...
class TestLargeVolumeHandling:
    ...
    def testpartition_volume_helper(self):
    ...
    def test_max_volume_checking(self):
        source = Trough('WaterTrough', virtual_rows=3, columns=3, min_volume=1000, max_volume=100 * 1000, initial_volumes=50 * 1000)
    ...
    def testpartition_by_columns_source(self):
        column_groups = partition_by_column(['A01', 'B01', 'A03', 'B03', 'C02'], ['A01', 'B01', 'C01', 'D01', 'E01'], [2500, 3500, 1000, 500, 2000], partition_by='source')
        assert len(column_groups) == 3
        assert column_groups[0] == (['A01', 'B01'], ['A01', 'B01'], [2500, 3500])
        assert column_groups[1] == (['C02'], ['E01'], [2000])
        assert column_groups[2] == (['A03', 'B03'], ['C01', 'D01'], [1000, 500])
        return

    def testpartition_by_columns_destination(self):
        column_groups = partition_by_column(['A01', 'B01', 'A03', 'B03', 'C02'], ['A01', 'B01', 'C02', 'D01', 'E02'], [2500, 3500, 1000, 500, 2000], partition_by='destination')
        assert len(column_groups) == 2
        assert column_groups[0] == (['A01', 'B01', 'B03'], ['A01', 'B01', 'D01'], [2500, 3500, 500])
        assert column_groups[1] == (['A03', 'C02'], ['C02', 'E02'], [1000, 2000])
        return

    def testpartition_by_columns_sorting(self):
        column_groups = partition_by_column(['A01', 'B01', 'B03', 'A03', 'C02'], ['B01', 'A01', 'C01', 'D01', 'E01'], [2500, 3500, 1000, 500, 2000], partition_by='source')
        assert len(column_groups) == 3
        assert column_groups[0] == (['A01', 'B01'], ['B01', 'A01'], [2500, 3500])
        assert column_groups[1] == (['C02'], ['E01'], [2000])
        assert column_groups[2] == (['A03', 'B03'], ['D01', 'C01'], [500, 1000])
        column_groups = partition_by_column(['A01', 'B01', 'B03', 'A03', 'C02'], ['B01', 'A01', 'C03', 'D03', 'E02'], [2500, 3500, 1000, 500, 2000], partition_by='destination')
        assert len(column_groups) == 3
        assert column_groups[0] == (['B01', 'A01'], ['A01', 'B01'], [3500, 2500])
        assert column_groups[1] == (['C02'], ['E02'], [2000])
        assert column_groups[2] == (['B03', 'A03'], ['C03', 'D03'], [1000, 500])
        return

class TestReagentDistribution:

    def test_parameter_validation(self, caplog):
        with BaseWorklist() as wl:
            with pytest.raises(ValueError):
    ...
    def test_full_parameterization(self):
        with BaseWorklist() as wl:
            wl.reagent_distribution('S1', 1, 20, 'D1', 2, 21, volume=50, diti_reuse=2, multi_disp=3, exclude_wells=[2, 4, 8], liquid_class='TestLC', direction='right_to_left', src_rack_type='MP3Pos', dst_rack_type='MP4Pos', src_rack_id='S1234', dst_rack_id='D1234')
    ...
    def test_large_volume_multi_disp_adaption(self):
        with BaseWorklist() as wl:
            wl.reagent_distribution('S1', 1, 8, 'D1', 1, 96, volume=400, multi_disp=6)
    ...
    def test_oo_example_1(self):
        src = Trough('T3', 8, 1, min_volume=20, max_volume=100 * 1000, initial_volumes=100 * 1000)
        ...
        with FluentWorklist() as wl:
            ...
            wl.distribute(src, 0, dst, dst_wells, volume=100, liquid_class='Water', diti_reuse=1, multi_disp=6, direction='left_to_right', src_rack_type='Trough 100ml', dst_rack_type='96 Well Microplate', label='Test Label')
        assert wl[0] == 'C;Test Label'
        assert wl[1] == 'R;T3;;Trough 100ml;1;8;MTP-96-3;;96 Well Microplate;1;96;100;Water;1;6;0;27;46;51;69;82'
    ...
    def test_oo_example_2(self):
        src = Trough('T2', 8, 1, min_volume=20, max_volume=100 * 1000, initial_volumes=100 * 1000)
        dst = Labware('MTP-96-2', 8, 12, min_volume=20, max_volume=300)
        with EvoWorklist() as wl:
            wl.distribute(src, 0, dst, dst.wells, volume=100, liquid_class='Water', diti_reuse=2, multi_disp=5, direction='left_to_right', src_rack_type='Trough 100ml', dst_rack_type='96 Well Microplate', label='Test Label')
        assert wl[0] == 'C;Test Label'
    ...
    def test_oo_block_from_right(self):
        src = Trough('Water', 8, 1, min_volume=20, max_volume=100 * 1000, initial_volumes=100 * 1000)
        dst = Labware('96mtp', 8, 12, min_volume=20, max_volume=300)
        with EvoWorklist() as wl:
            wl.distribute(src, 0, dst, dst.wells[1:4, 2:7], volume=50, liquid_class='TestLC', diti_reuse=10, multi_disp=5, direction='right_to_left', label='Test Label')
        assert wl[0] == 'C;Test Label'
