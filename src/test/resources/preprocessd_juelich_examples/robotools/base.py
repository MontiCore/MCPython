...
class BaseWorklist(list):
    ...
    def __init__(self, filepath=None, max_volume=950, auto_split=True):
    ...
    def _get_well_position(self, labware, well):
        """Internal method to resolve the well number for a given labware well."""
        raise TypeError('The use of a specific worklist type (typically EvoWorklist or FluentWorklist) is required for this operation.')
    ...
    def aspirate_well(self, rack_label, position, volume, *, liquid_class='', tip=Tip.Any, rack_id='', tube_id='', rack_type='', forced_rack_type=''):
        ...
        """
        (rack_label, position, volume_s, liquid_class, tip, rack_id, tube_id, rack_type, forced_rack_type) = prepare_aspirate_dispense_parameters(rack_label, position, volume, liquid_class, tip, rack_id, tube_id, rack_type, forced_rack_type, max_volume=self.max_volume)
        tip_type = ''
        self.append(f'A;{rack_label};{rack_id};{rack_type};{position};{tube_id};{volume_s};{liquid_class};{tip_type};{tip};{forced_rack_type}')
        return

    def dispense_well(self, rack_label, position, volume, *, liquid_class='', tip=Tip.Any, rack_id='', tube_id='', rack_type='', forced_rack_type=''):
        ...
        forced_rack_type : str, optional
            Overrides rack_type from worktable
        """
        (rack_label, position, volume_s, liquid_class, tipv, rack_id, tube_id, rack_type, forced_rack_type) = prepare_aspirate_dispense_parameters(rack_label, position, volume, liquid_class, tip, rack_id, tube_id, rack_type, forced_rack_type, max_volume=self.max_volume)
        tip_type = ''
        self.append(f'D;{rack_label};{rack_id};{rack_type};{position};{tube_id};{volume_s};{liquid_class};{tip_type};{tipv};{forced_rack_type}')
        return

    def reagent_distribution(self, src_rack_label, src_start, src_end, dst_rack_label, dst_start, dst_end, *, volume, diti_reuse=1, multi_disp=1, exclude_wells=None, liquid_class='', direction='left_to_right', src_rack_id='', src_rack_type='', dst_rack_id='', dst_rack_type=''):
        ...
        (src_rack_label, _, _, _, _, src_rack_id, _, src_rack_type, _) = prepare_aspirate_dispense_parameters(*src_args, max_volume=self.max_volume)
        dst_args = (dst_rack_label, 1, volume, '', Tip.Any, dst_rack_id, '', dst_rack_type, '')
        (dst_rack_label, _, _, _, _, dst_rack_id, _, dst_rack_type, _) = prepare_aspirate_dispense_parameters(*dst_args, max_volume=self.max_volume)
        ...
        self.append(f'R;{src_parameters};{dst_parameters};{volume};{liquid_class};{diti_reuse};{multi_disp};{direction_i}{exclude_str}')
        return

    def aspirate(self, labware, wells, volumes, *, label=None, **kwargs):
    ...
    def dispense(self, labware, wells, volumes, *, label=None, compositions=None, **kwargs):
    ...
    def transfer(self, source, source_wells, destination, destination_wells, volumes, *, label=None, wash_scheme=1, partition_by='auto', **kwargs):
        raise CompatibilityError('The transfer method is Fluent/Evo-specific, but this object is of the generic BaseWorklist type. Use an EvoWorklist or FluentWorklist for device-specific methods.')

    def distribute(self, source, source_column, destination, destination_wells, *, volume, diti_reuse=1, multi_disp=1, liquid_class='', label='', direction='left_to_right', src_rack_id='', src_rack_type='', dst_rack_id='', dst_rack_type=''):
        """Transfers from a Trough into many destination wells using multi-pipetting.
        ...
        self.comment(label)
        self.reagent_distribution(source.name, src_start, src_end, destination.name, dst_start, dst_end, volume=volume, diti_reuse=diti_reuse, multi_disp=multi_disp, exclude_wells=excluded_dst_wells, liquid_class=liquid_class, direction=direction, src_rack_id=src_rack_id, src_rack_type=src_rack_type, dst_rack_id=dst_rack_id, dst_rack_type=dst_rack_type)
        n_dst = len(dst_wells)
