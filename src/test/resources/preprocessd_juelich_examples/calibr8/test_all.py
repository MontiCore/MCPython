...
class TestDeprecatedClasses:
    ...
    def test_warns_numericposterior(self):
        with pytest.warns(DeprecationWarning, match='was renamed to `ContinuousUnivariateInference`'):
            arr = numpy.arange(10)
            calibr8.NumericPosterior(0.5, eti_x=arr + 0.1, eti_pdf=arr + 0.2, eti_prob=0.3, hdi_x=arr * 1.1 + 0.4, hdi_pdf=arr + 0.5, hdi_prob=0.6)
        pass
...
class TestInferenceResult:
    ...
    def test_univariate_continuous(self):
        arr = numpy.arange(10)
        pst = calibr8.ContinuousUnivariateInference(0.5, eti_x=arr + 0.1, eti_pdf=arr + 0.2, eti_prob=0.3, hdi_x=arr * 1.1 + 0.4, hdi_pdf=arr + 0.5, hdi_prob=0.6)
...
class TestNoiseModels:
    ...
    @pytest.mark.parametrize('cls,a,b,params', [(calibr8.NormalNoise, -1, 1, (0.2, 1.2)), (calibr8.LaplaceNoise, -1, 1, (-0.2, 0.8)), (calibr8.LogNormalNoise, 0.1, 2, (-0.1, 0.2)), (calibr8.StudentTNoise, -2, 10, (1.5, 0.3, 4)), (calibr8.PoissonNoise, 1, 7, (1.5,))])
    def test_parametrization_equivalence(self, cls, a, b, params):
    ...
    def test_none_dists_errors(self, which):
        ...
        with pytest.raises(NotImplementedError, match=which + '_dist.*?not defined'):
            cm.loglikelihood(x=numpy.arange(3), y=[1, 2, 3], theta=theta)
        pass
    ...
    def test_no_logpdmf_error(self):
        ...
        with pytest.raises(NotImplementedError, match='logpdf or logpmf methods'):
            cm.loglikelihood(x=numpy.arange(3), y=[1, 2, 3], theta=[5])
        pass
...
class TestBaseCalibrationModel:
    ...
    def test_constructor_signature_check(self):
        ...
        class EM_args(calibr8.ContinuousUnivariateModel):
            ...
            def __init__(self, arg1):
                super().__init__('I', 'D', theta_names=tuple('abc'))
        with pytest.raises(TypeError, match='constructor must not have any required \\(kw\\)arguments'):
            EM_args(arg1=3)
    ...
    def test_exceptions(self):
        ...
        with pytest.raises(NotImplementedError, match='does not implement an .infer_independent\\(\\)'):
...
class TestContinuousUnivariateModel:
    ...
    def test_continuous_univariate_exceptions(self):
        ...
        with pytest.raises(ValueError, match='Unexpected `ci_prob`'):
    ...
    @pytest.mark.parametrize('x', [numpy.array([1, 2, 3]), 4])
    @pytest.mark.parametrize('y', [numpy.array([2, 4, 8]), 5])
    ...
    def test_infer_independent(self):
        ...
        assert numpy.isclose(scipy.integrate.cumulative_trapezoid(pst.eti_pdf, pst.eti_x)[-1], 0.95, atol=0.0001)
        assert numpy.isclose(scipy.integrate.cumulative_trapezoid(pst.hdi_pdf, pst.hdi_x)[-1], 0.95, atol=0.0001)
...
class TestUnivariateInferenceHelpers:
    ...
    def test_get_eti(self):
        ...
        (eti_lower, eti_upper) = calibr8.core._get_eti(x_cdf=x_cdf, cdf=cdf, ci_prob=0.6827)
    ...
    def test_get_hdi(self, initial_guess):
        pdf = numpy.concatenate([numpy.linspace(0, 1e-06, 100), numpy.linspace(0, 1, 100), numpy.linspace(1e-06, 0, 100)])
        pdf = pdf / pdf.sum()
        ...
        (hdi_lower, hdi_upper) = calibr8.core._get_hdi(x_cdf=x, cdf=cdf, ci_prob=0.75, guess_lower=initial_guess[0], guess_upper=initial_guess[1], history=history)
...
class TestUtils:
    ...
    def test_datetime_parsing(self):
        ...
        assert calibr8.utils.parse_datetime('2018-12-01T09:27:30Z') == datetime.datetime(2018, 12, 1, 9, 27, 30, tzinfo=datetime.timezone.utc)
        assert calibr8.utils.parse_datetime('2018-12-01T09:27:30+0000') == datetime.datetime(2018, 12, 1, 9, 27, 30, tzinfo=datetime.timezone.utc)

    def test_datetime_formatting(self):
        assert calibr8.utils.format_datetime(None) is None
        assert calibr8.utils.format_datetime(datetime.datetime(2018, 12, 1, 9, 27, 30, tzinfo=datetime.timezone.utc)) == '2018-12-01T09:27:30Z'
    ...
    def test_istensor_with_pytensor(self):
        ...
        assert not calibr8.istensor([1, 2, 3])
        assert not calibr8.istensor('hello')
        test_dict2 = {'a': 1, 'b': [1, 2, 3], 'c': numpy.array([(1, at.as_tensor_variable([1, 2, 3])), (3, 4)])}
...
class TestOptimization:
    ...
    def _get_test_model(self):
        ...
        y = stats.t.rvs(loc=calibr8.polynomial(x, theta_mu), scale=calibr8.polynomial(x, theta_scale), df=theta[-1])
    ...
    def test_fit_scipy(self, caplog):
        ...
        (theta_mu, theta_scale, theta, em, x, y) = self._get_test_model()
        (theta_fit, history) = calibr8.fit_scipy(em, independent=x, dependent=y, theta_guess=numpy.ones_like(theta), theta_bounds=[(-5, 5)] * len(theta_mu) + [(0.02, 1), (1, 20)])
        for (actual, desired, atol) in zip(theta_fit, theta, [0.1, 0.05, 0.2, 2]):
        ...
        with caplog.at_level(logging.WARNING):
            x[0] = float('nan')
            y[-1] = numpy.inf
            calibr8.fit_scipy(em, independent=x, dependent=y, theta_guess=numpy.ones_like(theta), theta_bounds=[(-5, 5)] * len(theta_mu) + [(0.02, 1), (1, 20)])
        assert '2 elements' in caplog.text
        numpy.testing.assert_array_equal(em.cal_independent, x)
        numpy.testing.assert_array_equal(em.cal_dependent, y)
        pass
    ...
    def test_fit_scipy_global(self, caplog):
        ...
        (theta_mu, theta_scale, theta, em, x, y) = self._get_test_model()
        (theta_fit, history) = calibr8.fit_scipy_global(em, independent=x, dependent=y, theta_bounds=[(-10, 10)] * len(theta_mu) + [(0.001, 5), (1, 20)])
        for (actual, desired, atol) in zip(theta_fit, theta, [0.1, 0.05, 0.2, 2]):
            numpy.testing.assert_allclose(actual, desired, atol=atol)
        assert isinstance(history, list)
        numpy.testing.assert_array_equal(em.theta_fitted, theta_fit)
        ...
        with caplog.at_level(logging.WARNING):
            x[0] = float('nan')
            y[-1] = numpy.inf
            calibr8.fit_scipy_global(em, independent=x, dependent=y, theta_bounds=[(-10, 10)] * len(theta_mu) + [(0.001, 5), (1, 20)])
        assert '2 elements' in caplog.text
        numpy.testing.assert_array_equal(em.cal_independent, x)
        numpy.testing.assert_array_equal(em.cal_dependent, y)
        pass
    ...
    def test_global_solver_method_error(self):
        with pytest.raises(ValueError, match='supported global optimization solver method is'):
            (theta_mu, theta_scale, theta, em, x, y) = self._get_test_model()
            (theta_fit, history) = calibr8.fit_scipy_global(em, independent=x, dependent=y, theta_bounds=[(-10, 10)] * len(theta_mu) + [(0.001, 5), (1, 20)], method='direct')
            pass
    ...
    def test_warns_about_basemodelt(self):
        ...
        class UnmigratedModel(calibr8.BaseModelT):
            ...
            def __init__(self, independent_key='I', dependent_key='D'):
                super().__init__(independent_key=independent_key, dependent_key=dependent_key, theta_names=[])
...
class TestBaseModels:
    ...
    @pytest.mark.parametrize('cls,init_kwargs,theta,invertible', [(calibr8.BasePolynomialModelN, dict(mu_degree=1, sigma_degree=0), [1, 0.5, 0.1], True), (calibr8.BasePolynomialModelN, dict(mu_degree=1, sigma_degree=1), [1, 0.5, 0.1, 0.2], True), (calibr8.BasePolynomialModelN, dict(mu_degree=2, sigma_degree=0), [1, 0.5, 0.3, 0.1], False), (calibr8.BasePolynomialModelT, dict(mu_degree=1, scale_degree=0), [1, 0.5, 0.1, 3], True), (calibr8.BasePolynomialModelT, dict(mu_degree=1, scale_degree=1), [1, 0.5, 0.1, 0.2, 3], True), (calibr8.BasePolynomialModelT, dict(mu_degree=2, scale_degree=0), [1, 0.5, 0.3, 0.1, 3], False), (calibr8.BaseAsymmetricLogisticN, dict(sigma_degree=0), [-1, 2, 0.5, 0.2, 1, 0.2], True), (calibr8.BaseAsymmetricLogisticN, dict(sigma_degree=1), [-1, 2, 0.5, 0.2, 1, 0.2, 0.1], True), (calibr8.BaseAsymmetricLogisticT, dict(scale_degree=0), [-1, 2, 0.5, 0.2, 1, 0.2, 5], True), (calibr8.BaseAsymmetricLogisticT, dict(scale_degree=1), [-1, 2, 0.5, 0.2, 1, 0.2, 0.1, 5], True), (calibr8.BaseLogIndependentAsymmetricLogisticN, dict(sigma_degree=0), [-1, 2, 0.5, 0.2, 1, 0.2], True), (calibr8.BaseLogIndependentAsymmetricLogisticN, dict(sigma_degree=1), [-1, 2, 0.5, 0.2, 1, 0.2, 0.1], True), (calibr8.BaseLogIndependentAsymmetricLogisticT, dict(scale_degree=0), [-1, 2, 0.5, 0.2, 1, 0.2, 5], True), (calibr8.BaseLogIndependentAsymmetricLogisticT, dict(scale_degree=1), [-1, 2, 0.5, 0.2, 1, 0.2, 0.1, 5], True), (calibr8.BaseExponentialModelN, dict(sigma_degree=0), [-1, 5, 1, 0.1], True), (calibr8.BaseExponentialModelN, dict(sigma_degree=1), [2, 1, 0.4, 0.1, 0.5], True), (calibr8.BaseExponentialModelT, dict(scale_degree=0), [-1, 5, 1, 0.1, 3], True), (calibr8.BaseExponentialModelT, dict(scale_degree=1), [2, 1, 0.4, 0.1, 0.5, 3], True), (calibr8.BaseExponentialModelN, dict(sigma_degree=0, fixed_intercept=1), [-5, 1, 0.1], True), (calibr8.BaseExponentialModelT, dict(scale_degree=0, fixed_intercept=-1), [-2, 1, 0.1, 3], True)])
