...
class BasePolynomialModelN(core.ContinuousUnivariateModel, noise.NormalNoise):
    ...
    def __init__(self, *, independent_key, dependent_key, mu_degree, sigma_degree=0, theta_names=None):
        ...
        if theta_names is None:
            theta_names = tuple((f'mu_{d}' for d in range(mu_degree + 1))) + tuple((f'sigma_{d}' for d in range(sigma_degree + 1)))
...
class BaseAsymmetricLogisticN(core.ContinuousUnivariateModel, noise.NormalNoise):
    ...
    def __init__(self, *, independent_key, dependent_key, sigma_degree=0, theta_names=None):
        ...
        self.sigma_degree = utils.check_scale_degree(sigma_degree)
        if theta_names is None:
            theta_names = tuple('L_L,L_U,I_x,S,c'.split(',')) + tuple((f'sigma_{d}' for d in range(sigma_degree + 1)))
...
class BaseLogIndependentAsymmetricLogisticN(core.ContinuousUnivariateModel, noise.NormalNoise):
    ...
    def __init__(self, *, independent_key, dependent_key, sigma_degree=0, theta_names=None):
        ...
        self.sigma_degree = utils.check_scale_degree(sigma_degree)
        if theta_names is None:
            theta_names = tuple('L_L,L_U,log_I_x,S,c'.split(',')) + tuple((f'sigma_{d}' for d in range(sigma_degree + 1)))
        super().__init__(independent_key, dependent_key, theta_names=theta_names)
...
class BaseExponentialModelN(core.ContinuousUnivariateModel, noise.NormalNoise):
    ...
    def __init__(self, *, independent_key, dependent_key, sigma_degree=0, fixed_intercept=None, theta_names=None):
