...
class BaseModelT(core.ContinuousUnivariateModel, noise.StudentTNoise):
    ...
    def __init__(self, independent_key, dependent_key, *, theta_names):
        warnings.warn('The`BaseModelT` class is deprecated. Inherit `core.ContinuousUnivariateModel, noise.StudentTNoise` directly.', FutureWarning)
...
class BasePolynomialModelT(core.ContinuousUnivariateModel, noise.StudentTNoise):
    ...
    def __init__(self, *, independent_key, dependent_key, mu_degree, scale_degree=0, theta_names=None):
        ...
        if theta_names is None:
            theta_names = tuple((f'mu_{d}' for d in range(mu_degree + 1))) + tuple((f'scale_{d}' for d in range(scale_degree + 1))) + ('df',)
        super().__init__(independent_key=independent_key, dependent_key=dependent_key, theta_names=theta_names)
...
class BaseAsymmetricLogisticT(core.ContinuousUnivariateModel, noise.StudentTNoise):
    ...
    def __init__(self, *, independent_key, dependent_key, scale_degree=0, theta_names=None):
        ...
        self.scale_degree = utils.check_scale_degree(scale_degree)
        if theta_names is None:
            theta_names = tuple('L_L,L_U,I_x,S,c'.split(',')) + tuple((f'scale_{d}' for d in range(scale_degree + 1))) + ('df',)
...
class BaseLogIndependentAsymmetricLogisticT(core.ContinuousUnivariateModel, noise.StudentTNoise):
    ...
    def __init__(self, *, independent_key, dependent_key, scale_degree=0, theta_names=None):
        ...
        self.scale_degree = utils.check_scale_degree(scale_degree)
        if theta_names is None:
            theta_names = tuple('L_L,L_U,log_I_x,S,c'.split(',')) + tuple((f'scale_{d}' for d in range(scale_degree + 1))) + ('df',)
        super().__init__(independent_key, dependent_key, theta_names=theta_names)
...
class BaseExponentialModelT(core.ContinuousUnivariateModel, noise.StudentTNoise):
    ...
    def __init__(self, *, independent_key, dependent_key, scale_degree=0, fixed_intercept=None, theta_names=None):
