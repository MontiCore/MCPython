...
def _mask_and_warn_inf_or_nan(x, y, on=None):
...
def fit_scipy(model, *, independent, dependent, theta_guess, theta_bounds, minimize_kwargs=None):
    ...
    history = []
    fit = scipy.optimize.minimize(model.objective(independent=independent_finite, dependent=dependent_finite, minimize=True), x0=theta_guess, bounds=theta_bounds, callback=lambda x: history.append(x), **minimize_kwargs)
    if theta_bounds:
...
def fit_scipy_global(model, *, independent, dependent, theta_bounds, method=None, maxiter=5000, minimizer_kwargs=None):
    ...
    if method.lower() != 'dual_annealing':
        raise ValueError(f'`{method.lower()}` is not supported. The supported global optimization solver method is `dual_annealing`.')
    history = []
    fit = scipy.optimize.dual_annealing(model.objective(independent=independent_finite, dependent=dependent_finite, minimize=True), bounds=theta_bounds, callback=lambda x, f, context: history.append((x, f, context)), maxiter=maxiter)
    if theta_bounds:
        bound_hit = _warn_hit_bounds(fit.x, theta_bounds, model.theta_names)
    if not fit.success or bound_hit:
        _log.warning(f'Fit of {type(model).__name__} has failed:')
