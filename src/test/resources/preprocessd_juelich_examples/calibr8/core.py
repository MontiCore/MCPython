...
class ContinuousInference(InferenceResult):
    ...
    def __init__(self, eti_x, eti_pdf, eti_prob, hdi_x, hdi_pdf, hdi_prob):
...
class ContinuousUnivariateInference(ContinuousInference):
    ...
    def __init__(self, median, **kwargs):
        """The result of a numeric infer_independent operation.
...
def _get_eti(x_cdf, cdf, ci_prob):
...
def _get_hdi(x_cdf, cdf, ci_prob, guess_lower, guess_upper, *, history=None):
    ...
    fit = scipy.optimize.fmin(hdi_objective, x0=initial_guess, xtol=numpy.ptp(x_cdf) / len(x_cdf), disp=False)
...
def _inherits_noisemodel(cls):
    ...
    for m in cls.__mro__:
        if issubclass(m, DistributionMixin) and m is not DistributionMixin and (not issubclass(m, CalibrationModel)):
            return True
    return False
...
class CalibrationModel(DistributionMixin):
    ...
    def __init__(self, independent_key, dependent_key, *, theta_names, ndim):
        ...
        if not _inherits_noisemodel(type(self)):
            warnings.warn('This model does not implement a noise model yet.\nAdd a noise model mixin to your class definition. For example:\n`class MyModel(CalibrationModel, LaplaceNoise)`', DeprecationWarning, stacklevel=2)
        ...
        self.__theta_names = tuple(theta_names)
        ...
        self.cal_independent = None
        self.cal_dependent = None
        super().__init__()
    ...
    @property
    def theta_fitted(self):
        """The parameter vector that describes the fitted model."""
        return self.__theta_fitted

    @theta_fitted.setter
    def theta_fitted(self, value):
        if value is not None:
            if numpy.shape(value) != numpy.shape(self.theta_names):
                raise ValueError(f'The number of parameters ({len(value)}) does not match the number of parameter names ({len(self.theta_names)}).')
    ...
    def infer_independent(self, y, *, lower, upper):
    ...
    def loglikelihood(self, *, y, x, name=None, replicate_id=None, dependent_key=None, theta=None, **dist_kwargs):
        ...
        **dist_kwargs : dict
        ...
        if not isinstance(x, (list, numpy.ndarray, float, int)) and (not utils.istensor(x)):
            raise ValueError(f'Input x must be a scalar, TensorVariable or an array-like object, but not {type(x)}')
        ...
        if utils.istensor(x) or utils.istensor(theta):
            ...
            if pmodel is not None:
                if replicate_id and dependent_key:
                    warnings.warn('The `replicate_id` and `dependent_key` parameters are deprecated. Use `name` instead.', DeprecationWarning)
    ...
    def likelihood(self, *, y, x, theta=None, scan_x=False):
    ...
    def save(self, filepath):
        ...
        """
        data = dict(calibr8_version=__version__, model_type='.'.join([self.__module__, self.__class__.__qualname__]), theta_names=self.theta_names, theta_bounds=self.theta_bounds, theta_guess=self.theta_guess, theta_fitted=self.theta_fitted, theta_timestamp=utils.format_datetime(self.theta_timestamp), independent_key=self.independent_key, dependent_key=self.dependent_key, cal_independent=self.cal_independent.tolist() if self.cal_independent is not None else None, cal_dependent=self.cal_dependent.tolist() if self.cal_dependent is not None else None)
    ...
    def load(cls, filepath):
        ...
        if cls.__name__ not in json_type:
            raise utils.CompatibilityException(f'The model type from the JSON file ({json_type}) does not match this class ({cls.__name__}).')
        obj = cls()
        ...
        obj.__theta_guess = tuple(data['theta_guess']) if 'theta_guess' in data and data.get('theta_guess') is not None else None
...
def infer_independent(*, likelihood, y, lower, upper, steps=300, ci_prob=1):
    """Infer the posterior distribution of the independent variable given the observations of the dependent variable.
    ...
    (likelihood_integral, _) = scipy.integrate.quad(func=lambda x: likelihood(x=x, y=y), a=lower, b=upper)
    x_integrate = numpy.linspace(lower, upper, 10000)
    area = scipy.integrate.cumulative_trapezoid(likelihood(x=x_integrate, y=y, scan_x=True), x_integrate, initial=0)
    cdf = area / area[-1]
    shrink = 1e-05
    (xfrom, xto) = _get_eti(x_integrate, cdf, 1 - shrink)
    x_integrate = numpy.linspace(xfrom, xto, 100000)
    area = scipy.integrate.cumulative_trapezoid(likelihood(x=x_integrate, y=y, scan_x=True), x_integrate, initial=0)
    ...
    return ContinuousUnivariateInference(median, eti_x=eti_x, eti_pdf=eti_pdf, eti_prob=eti_prob, hdi_x=hdi_x, hdi_pdf=hdi_pdf, hdi_prob=hdi_prob)

class ContinuousUnivariateModel(CalibrationModel):

    def __init__(self, independent_key, dependent_key, *, theta_names):
        super().__init__(independent_key, dependent_key, theta_names=theta_names, ndim=1)

    def infer_independent(self, y, *, lower, upper, steps=300, ci_prob=1):
        """Infer the posterior distribution of the independent variable given the observations of the dependent variable.
        The calculation is done numerically by integrating the likelihood in a certain interval [upper,lower].
        This is identical to the posterior with a Uniform (lower,upper) prior.
        ...
        posterior : ContinuousUnivariateInference
            Result of the numeric posterior calculation.
        """
        return infer_independent(likelihood=self.likelihood, y=y, lower=lower, upper=upper, steps=steps, ci_prob=ci_prob)
...
class ContinuousMultivariateModel(CalibrationModel):
    ...
    def infer_independent(self, y, *, lower, upper):
        raise NotImplementedError('ContinuousMultivariateModel does not implement an .infer_independent() method. Consult the documentation to learn how to perform inference with a PyMC model.')
